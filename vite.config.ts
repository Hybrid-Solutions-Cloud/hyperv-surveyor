import { defineConfig } from 'vite'
import react from '@vitejs/plugin-react'

export default defineConfig({
  plugins: [react()],
  base: './',
  build: {
    outDir: 'dist',
    sourcemap: false,
    chunkSizeWarningLimit: 4000,
    // IIFE, not ESM. Browsers refuse to load ES modules over file://, so a module build
    // cannot be opened by double-clicking. This makes the single-file bundle work offline.
    rollupOptions: {
      output: {
        format: 'iife',
        inlineDynamicImports: true,
        // GitHub Pages caches JavaScript much longer than HTML. A content hash forces
        // every deployment to use a new URL instead of leaving visitors on stale app.js.
        entryFileNames: 'app.[hash].js',
        assetFileNames: 'app.[hash].[ext]',
      },
    },
  },
})
